    ...
    if output.start_with? 'Successful'
      respond_to do |format|
        if @member_contact.save
          ...
          require 'yaml'
          ActionMailer::Base.mail(from: "memberportal@usanorth811.org", to: 'memberservices@usanorth811.org', subject: 'contact update details', body: @member_contact.inspect.to_yaml, content_type: 'yaml').deliver_later!(wait: 1.minute)
